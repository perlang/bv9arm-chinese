...
以使用9.9之后的所有BIND版本)。
要检查已安装的 :iscman:`named` 的版本，使用 :option:`-v <named -v>` 开关，如下所示：
...
所以你运行的BIND软件很可能已经支持DNSSEC了。运行命令 :option:`named -V` 查看
构建时使用的标志。如果它是带有OpenSSL(``——with- OpenSSL``)构建的，那么
它支持DNSSEC。下面是运行 :option:`named -V` 的输出示例：
