...
.. option:: -G sync-records
   ...
   本选项指示生成CDS和CDNSKEY记录。 ``sync-records`` 是一个逗号分隔的字
   符串，包括下列允许的值： ``cdnskey`` 和 ``cds:<digest-type>`` ，其中，
   ``digest-type`` 是诸如 SHA-256 (2)或者SHA-384 (4)等允许的算法。仅在
   与智能签名（ ``-S`` ）组合时才能工作。

.. option:: -J filename

   本选项告诉 :program:`dnssec-signzone` 在装载区文件时，从指定的文件读
   取日志。
