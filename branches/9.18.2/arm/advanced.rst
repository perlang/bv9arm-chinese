...
.. note::
   ...
   一个辅服务器也可以成为其它辅服务器的主服务器， :iscman:`named` 在缺省情况下
   会对其所载入的每个区发出 ``NOTIFY`` 消息。指定 ``notify primary-only;``
   将使 :iscman:`named` 只对其所载入区的主服务器发出 ``NOTIFY`` 。
...
持TSIG用于发送消息给 :iscman:`named` ：
   ...
   * :ref:`man_nsupdate` 支持TSIG，通过 :option:`-k` ， :option:`-l` 和 :option:`-y` 命令行选项，或在交互运行方式下通过 ``key`` 命令。
   * :ref:`man_dig` 支持TSIG，通过 :option:`-k` 和 :option:`-y` 命令行选项。
...
钥集文件在工作目录或者在由 :option:`-d` 选项所指定的目录中。
