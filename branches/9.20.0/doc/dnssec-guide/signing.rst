...
使用 :any:`dnssec-policy` 开启动态DNS或 :any:`inline-signing` 。
...
录 [#rrsig]_ 都会至少带有一条对应的签名，也被称为RRSIG记录。
...
.. [#rrsig]
   嗯，几乎所有记录：NS记录和授权的粘连记录没有RRSIG记录。如
   果没有授权，你区中的每个记录都会被签名并带有其自己的RRSIG。
...
:any:`dnssec-policy` 是在一个区中运行DNSSEC的首选方式，但有时还是需要一
个更加“事必躬亲”的方式来进行签名和密钥维护。基于这个原因，我们将在
:ref:`advanced_discussions_manual_signing` 中介绍手动的签名技术。
...
这个例子也将 :any:`checkds` 设置为 `explicit` 。这意味着仅在
:any:`parental-agents` 中定义的地址才会被查询。如果设置为 `yes` ，会查
询父区的NS记录来查找父区代理。
...
当 :any:`checkds` 被设置为 `no` 时，也可以使用这个命令。
...
手动签名是引入BIND的第一种方法，并且如其名字所暗示，不提供自动化。用
户需要做所有的事情：创建密钥，使用它们对区签名，加载已签名的区，周期
性地重签区，并管理密钥的轮转，还包括与父区的交互。用户当然可以完成所
有这些，但是为什么不使用自动化的方法呢？
   
虽然使用自动的 :any:`dnssec-policy` 是BIND中签名区的首选方法，但在某些
情况下，可能需要手动的方法。 :any:`dnssec-policy` 目前不支持使用外部硬
件，所以如果您的安全策略需要它，您需要使用手动签名。

BIND 9附带了几个用于这个过程的工具，这将在下面详细解释。在所有的情况下，
``-h`` 选项都会打印出一个完整的参数列表。注意，DNSSEC工具要求密钥集文
件在工作目录或者在由 ``-d`` 选项所指定的目录中。

要将一个传统的（不安全的）DNS区转化为一个安全的区，我们需要创建各种附加
记录（DNSKEY，RRSIG，NSEC/NSEC3），进行完全自动化的签名，并将可验证的信
息（如一条DS记录）上传到父区以完成信任链。

第一步是按照 :ref:`manual_signing_generate_keys` 中的描述创建密钥，然后
使用BIND提供的工具 :iscman:`dnssec-keygen` 创建密钥，并用
:iscman:`dnssec-signzone` 对区签名。签名区存放在另一个文件中，也即告诉
BIND需要加载的文件。要更新区（例如，增加一条资源记录），你需要更新未签
名的区，重签这个区，并告诉 :iscman:`named` 加载已经更新的签名副本。对于
刷新签名和轮转密钥也是如此；用户负责提供已签名的区给 :iscman:`named` 用
于服务。（在轮转密钥的情况，你还负责确保在适当的时间添加和删除密钥。）

为什么你要以这种方式来签名区？通常你大概不会这样做，但可能在某些情况需
要这样做，所以这些脚本就被保留在BIND发行版中了。
...
.. note::
   ...
   安全的BIND安装，使用诸如监禁过程 [#JP]_ 和限制用户特权等技术，但这些都
...
在这两对密钥中，一个是区签名密钥(ZSK)，一个是密钥签名密钥(KSK) [#onekey]_ 。我们可
...
密钥文件中存有与轮转密钥相关的时间信息。这是在创建文件时由
:iscman:`dnssec-keygen` 生成的，可以使用 :iscman:`dnssec-settime` 来修改。
缺省情况下，文件中只包含有限的定时信息，如前一节中的示例所示。

注意， :any:`dnssec-policy` 设置密钥的时间信息，但它使用自身的状态机来决
定执行什么动作。

但是在使用密钥文件中的密钥参数和时间信息进行手工签名时，你可以为区执行任
何想要的DNSSEC策略。

所有的日期都是相同的，并且是 :iscman:`dnssec-keygen` 创建密钥时的日期和时间。
我们可以使用 :iscman:`dnssec-keygen` 来修改日期 [#moddate]_ 。例如，要在2020年7月1
...
发布日期是密钥应该被引入区的日期。激活日期用于决定何时对资源记录进行签名。
在签名者应该停止使用指定密钥生成新签名时，你发出“Inactive”信号，而
"Delete"元数据指定何时将密钥从区中移除。

最后，我们应该注意到 :iscman:`dnssec-keygen` 命令支持同样的开关集，所以我们可
以在创建密钥时设置日期。

.. [#onekey]
   只需要一个密钥文件 - 一个KSK或者ZSK - 以表明区的存在。
   :iscman:`dnssec-keygen` 可以按需创建两种类型的文件。
...
现在，编辑区文件并确保其包含了正确的DNSKEY条目。应当通过使用 ``$INCLUDE``
语句包含 ``.key`` 文件而将公钥插入到区文件中。

使用命令 :iscman:`dnssec-signzone` 。应当出现一些对应于安全子区的 ``keyset``
文件。区签名者为区生成 ``NSEC`` ， ``NSEC3`` 和 ``RRSIG`` 记录，如果指定了
:option:`-g <dnssec-signzone -g>` ，还为子区生成 ``DS`` 记录。如果未指定
:option:`-g <dnssec-signzone -g>` ，就必须为安全的子区手工添加DS资源记录集。

下列命令对区签名，假设它在一个名为 ``zone.child.example`` 的文件中，使用手
工指定的密钥：
...
:iscman:`dnssec-signzone` 也生成keyset和dsset文件。这些用于向父区管理
员提供 ``DNSKEY`` 记录（或它们对应的 ``DS`` 记录），用作区的安全入口
点。

缺省时，所有的区密钥有一个可用的私钥用于生成签名。你可以使用
:option:`-S <dnssec-signzone -S>` 包含那些在过去已有"Activate"时间元
数据的密钥和在将来（或者不是现在）具有"Inactive"时间元数据的密钥。
...
现在是时候通知BIND一个新的已签名区文件准备好了。我们可以使用
:option:`rndc reload example.com <rndc reload>` 来完成这件事。
...
你现在应当检查区已被签名，按照 :ref:`signing_verification` 中的步骤。
...
如在 :ref:`signing_easy_start_upload_to_parent_zone` 中的描述，我们现
在必须上载新的信息给父区。信息的格式和任何生成它已在
:ref:`working_with_parent_zone` 中描述，重要的是记住你必须使用在上文
中（作为处理过程的一部份）所生成的KSK文件。

文件 ``dsset-example.com`` （由 :iscman:`dnssec-signzone` 在对区
``example.com`` 签名时创建）包含区的KSK对应的DS记录。

如果还未做，你将其传递给父区的管理员，以放置在父区中。当DS记录在父区中
发布，你的区才是完全签名了。
...
最后，按照 :ref:`how_to_test_authoritative_server` 中的步骤确认一个
查询识别区是被父区正确签名和证明的。
...
由于这是一个手动过程，您将需要周期性重新签名，以及每次区数据变化时都需要
...
一旦将区签名，就必须如同 :ref:`signing_maintenance_tasks` 中所描述的对其
进行监控。无论如何，一个密钥接近轮转时间时，你必须创建一个新密钥。当然，
也可以一次创建接下来五十年的密钥并设置合适的密钥时间。
将未来密钥的私钥文件存放在磁盘上增加的风险是否能够抵消在轮换之前必须记得
创建新密钥而增加的工作，这取决于你的组织的安全策略。
