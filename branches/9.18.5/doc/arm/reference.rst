...
.. _primaries:
    ...
    :any:`http`
        为一个HTTP连接指定配置信息，包括 ``endponts`` ， :any:`listener-clients` 和 :any:`streams-per-connection` 。
...
:any:`parental-agents` 列表允许一个通用的父区代理集合能够很容易地被多
个主区和辅区用于其 :any:`parental-agents` 列表中。父区代理是一个区与之
有联系并通过其修改自己的授权信息的实体（在 :rfc:`7344` 中定义）。
...
.. namedconf:statement:: random-device
   ...
   如果未指定 :any:`random-device` ，或者它被设置为 ``none`` ，将会从 BIND 所链
   接到的加密库（即，OpenSSL 或者一个 PKCS#11 提供者）所提供的随机数生成函数
   中读取熵。

   :any:`random-device` 选项在服务器启动时的初始配置装载时生效，而在随后的重载
   将会忽略它。
...
.. namedconf:statement:: dns64
   ...
   这条指令指示 :iscman:`named` 在没有找到AAAA记录时，返回由IPv4地址所
   映射到的AAAA查询。其意图是用于和NAT64相配合。每个 :any:`dns64` 定义
   一个DNS64前缀。可以定义多个DNS64前缀。
   ...
   .. namedconf:statement:: mapped
      ...
      每个 :any:`dns64` 支持一个可选的 :any:`mapped` 访问控制表，它在
      相关的A资源记录集中选择哪些IPv4地址会被映射。如果未设置，缺省值
      为 ``any;`` 。
...
.. namedconf:statement:: max-zone-ttl
   ...
   可以用于指定最大值。在使用一个 :any:`masterfile-format` 为 ``text`` 或
   者 ``raw`` 装载一个区文件时，任何带有超过 :any:`max-zone-ttl` 的TTL值的
   记录都将导致区被拒绝。
   ...
   在 :namedconf:ref:`options` 和 :namedconf:ref:`zone` 块中，
   缺省值是 ``unlimited`` 。一个为0的 :any:`max-zone-ttl` 被当成
   ``unlimited`` 看待。

   在 :namedconf:ref:`dnssec-policy` 块中，缺省值是 ``PT24H`` （24小时
   ）。一个为0的 :any:`max-zone-ttl` 被当成使用了缺省值看待。
...
.. namedconf:statement:: dialup
   ...
   :any:`heartbeat-interval` 进行一次，理想情况在一次连接中完成。它也
...
.. namedconf:statement:: root-key-sentinel
   ...
   如果为 ``yes`` ，按照draft-ietf-dnsop-kskroll-sentinel-08所描述的响
   应对根密钥标记的探测。缺省是 ``yes`` 。
...
.. namedconf:statement:: check-integrity
   ...
   对NS记录，仅对区的顶端下的名字进行检查（对区外名字和粘着一致性检查
...
.. namedconf:statement:: blackhole
   :tags: query
   :short: 定义一个服务器不会响应也不会去查询的主机的 :any:`address_match_list` 。
...
.. namedconf:statement:: clients-per-query
   ...
   设置允许同时进行的针对任何给定请求（<qname，qtype，qclass>）的递
   归客户端的初始数目（最小值），对任何超过这个数目的额外请求，
   ...
   如果 :any:`clients-per-query` 设为零，表示每个请求的客户端没有限制，没
   有请求被扔掉。
...
.. namedconf:statement:: max-clients-per-query
   ...
   如果 :any:`max-clients-per-query` 设为零，表示除了 :any:`recursive-clients`
   所设定的值外没有上限。
...
.. namedconf:statement:: tcp-advertised-timeout
   ...
   这个设置服务器在包含EDNS TCP keepalive选项的响应中发送的超时值（100
...
.. namedconf:statement:: max-udp-size
   ...
   设置 :any:`max-udp-size` 为非缺省值的通常的原因是允许UDP回答穿过坏的防
   火墙，这样的防火墙阻止碎包和（或）阻止大于512字节的UDP包通过。这是独
   立于所广播的接收缓存（ :any:`edns-udp-size` ）。
...
.. namedconf:statement:: masterfile-format
   ...
   省略某些只对 ``text`` 格式文件才做的检查。例如， :any:`check-names` 只
   用于装载 ``text`` 格式的区，而 :any:`max-zone-ttl` 只用于 ``text`` 和
...
.. namedconf:statement:: masterfile-style
   ...
   这指定当 :any:`masterfile-format` 为 ``text`` ，在导出时指定区文件的格式。
...
.. namedconf:statement:: transfers
   ...
   :any:`transfers` 用于限制从指定的服务器进来的并发区传送的数目。如果
...
:any:`max-zone-ttl`
    ...
    与 :namedconf:ref:`max-zone-ttl` 区选项相似，这为区指定了以秒为单
    位的最大允许的TTL值。
...
.. namedconf:statement:: parental-source
   ...
   送父区DS请求。这个语句为所有区设置 :any:`parental-source` ，但是可以通
...
.. namedconf:statement:: file
   ...
   这个设置区的文件名，在 :any:`primary <type primary>` ，
...
殊的问题可能是一件令人畏惧的任务。为了简化这个任务，所有语句都被分配了
...
语句
----
下列表格列出了在 :file:`named.conf` 中允许的所有语句。请注意本部份是正
在进行中的工作。
...
``QryDropped``
   ...
   后被丢弃的巨量递归查询的数量。这是由于 :any:`clients-per-query`
   和 :any:`max-clients-per-query` 选项（参见
