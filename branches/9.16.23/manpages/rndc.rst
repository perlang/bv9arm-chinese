...
``-c config-file``
   本选项指示 ``config-file`` 作为缺省的配置文件 ``/etc/rndc.conf`` 的
   替代。

``-k key-file``
   本选项指示 ``key-file`` 作为缺省的密钥文件 ``/etc/rndc.key`` 的替代。
   如果config-file不存在， ``/etc/rndc.key`` 中的密钥将用于认证发
   向服务器的命令。
...
``serve-stale`` (**on** | **off** | **reset** | **status**) [*class* [*view*]]
   ...
   ``rndc serve-stale status`` 报告旧答复服务当前是被配置打开或关
