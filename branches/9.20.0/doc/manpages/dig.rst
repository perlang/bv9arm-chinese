...
.. option:: -k keyfile
   ...
   本选项告诉 :program:`dig` 使用TSIG或SIG(0)对查询签名，使用一个从给
   定的文件中读到的一个密钥。密钥文件可以使用 :iscman:`tsig-keygen` 生
   成。在与 :program:`dig` 之间使用TSIG认证时，被请求的名字服务器需要
   知道所使用的密钥和算法。在BIND中，通过在 :iscman:`named.conf` 中为
   TSIG提供合适的 ``key`` 和 ``server`` 语句已经在区数据中为SIG(0)查找
   KEY记录来完成。
...
.. option:: +fuzztime[=value], +nofuzztime
   ...
   本选项在生成签名消息是指定签名时间。如果指定了一个值，它是自UTC 1970
   年1月1日00:00:00起的秒数，忽略闰秒。如果未指定值，使用1646972129（
   UTC 2022年3月11日04:15:29）。缺省为 ``+nofuzztime`` 并使用当前时间。
...
.. option:: +idn, +noidn
   ...
   开启或关闭对IDN的处理。缺省时，对于输入的查询名，以及输出到终端的显示，
   IDN是开启的。

   你也可以通过设置 ``IDN_DISABLE`` 环境变量关闭 :program:`dig` 的IDN处理。
...
.. option:: +proxy[=src_addr[#src_port]-dst_addr[#dst_port]], +noproxy
   ...
   当设置了本选项时， :program:`dig` 向查询中添加PROXYv2头部。指定了源
   地址和目的地址时，头部中会包含二者并使用 ``PROXY`` 命令。对于远程对
   等端来说，这意味着查询是代表另一个节点发送的，并且PROXYv2头部反映了
   原始连接端点。缺省的源端口是 ``0`` ，目的端口是 `53` 。

   对于加密的DNS传输，为阻止偶然的信息泄露，加密会应用到PROXYv2头部：
   在握手过程完成后才发送头部。

   对于普通的DNS传输，不会应用加密到PROXYv2头部。

   如果省略了地址，则添加使用 ``LOCAL`` 命令集的PROXYv2头部。对于远程
   对等端，这意味着查询是故意发送的，而不需要中继，因此必须使用真实的
   连接端点地址。

.. option:: +proxy-plain[=src_addr[#src_port]-dst_addr[#dst_port], +noproxy-plain

   与 ``+[no]proxy`` 相似，但是指示 ``dig`` 在任何加密之前，在发送任何
   握手信息之前发送PROXYv2头部。这使得 :program:`dig` 恰似在PROXY协议
   规范中所描述的一致，但并非所有软件都期待如此行为。

   如果你需要本选项，请参考软件文档以找到答案。（例如， ``dnsdist``
   期望当使用加密时通过TLS发送PROXYv2头部，而 ``HAProxy`` 及其它许多
   软件包期望普通的传输。）

   对于普通的DNS传输，本选项在效果上相当于上述 ``+[no]proxy`` 的别名。
...
.. option:: +tcp, +notcp
   ...
   AXFR请求总是使用TCP。要在一个UDP查询返回TC=1时阻止使用TCP重试，使用
   ``+ignore`` 。
...
.. option:: +trace, +notrace
   ...
   当设置了 :option:`+trace` 时，也会设置 :option:`+dnssec` ，来更好地模仿来自某
   个名字服务器的缺省请求。

   注意 ``delv +ns`` 选项也可用于跟踪一个名字从根开始的解析（参见
   :iscman:`delv` ）。
...
记录的查找。应用了一个全局请求选项 :option:`+qr` ，这样 :program:`dig`
显示它所进行的每个查找的初始请求。最终的请求有一个局部请求选项
:option:`+noqr` ，表示 :program:`dig` 不会打印它在查找 ``isc.org`` 的
NS记录时的初始请求。
