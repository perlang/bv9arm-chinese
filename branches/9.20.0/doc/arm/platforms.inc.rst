...
通常，本版本的BIND可以在任何带有现代C11（或更好的）编译器、RFC兼容的IPv6支持
的BSD风格套接字、POSIX兼容的线程和
:ref:`要求的库 <build_dependencies>` 的POSIX兼容的系统上构建和运行。

编译BIND 9要求下列C11特性：

- 定义在<stdatomic.h>中的对原子操作的支持

- 定义在<threads.h>中的对线程本地存储的支持

在有意义的地方，BIND 9使用了由C11标准的C17更新引入的C标准修复。
...
-  超过或接近其各自的生存期的平台，例如：
   ...
   -  Ubuntu 14.04, 16.04, 18.04 (不再支持Ubuntu ESM版本)
