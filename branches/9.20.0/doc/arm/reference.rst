...
.. glossary::
   ...
   Statement
      ...
      - 语句可以有单一参数（一个 **值** ）或者多个参数（ **参数/值**
        ...
        是对应的值。
      - 语句可以出现在一个单一的 :term:`block <Block>` 中 - 例如，一个
...
:any:`allow-update` ， :any:`allow-update-forwarding` ，和 :any:`blackhole`
都使用地址匹配表。类似地，:any:`listen-on` 选项
...
.. glossary::
    ...
    ``duration``
        在BIND 9中，一段持续时间可以写成三种方式：以一个数字表示的秒数，以一
        个带TTL风格的时间单位后缀的数字字符串，或以ISO 6801持续时间格式。
 
        可以允许的TTL时间单位后缀有：“W”（周），“D”（天），“H”（小时），
        “M”（分）和“S”（秒）。例如：“1W”（1周），“3d12h”（3天，12小时）。
 
        ISO 8601持续时间格式由字母“P”和后跟一个可选的系列数字和单位后缀“Y”
        （年），“M”（月），“W”（周）和“D”（天）组成；作为可选项，后面还可以
        后跟字母“T”和另一个系列的数字及单位后缀“H”（小时），“M”（分）和“S”
        （秒）。例如：“P3M10D”（3月，10天）。“P2WT12H”（2周，12小时），
        “pt15m”（15分）。更多关于ISO 8601持续时间格式的信息，参见
        :rfc:`3339` ，附录A。
 
        TTL风格和ISO 8601持续时间格式都是大小写无关的。
...
以下是所支持的块：
    ...
    :any:`dnssec-policy`
        为区描述一个 DNSSEC 密钥和签名策略。详细信息参见 :any:`dnssec-policy` 。

    :namedconf:ref:`key`
        指定在使用TSIG时，用于认证和授权的密钥信息。

    :any:`key-store`
        描述一个DNSSEC密钥存储位置。详细信息参见 :ref:`key-store Grammar <key_store_grammar>` 。
...
``key`` 语句定义了一个共享密钥，用于 TSIG （参见 :ref:`tsig` ）或命令
通道（参见 :any:`controls` ）中。

``key`` 语句可以放在配置文件的顶级或一个 :any:`view` 语句的内部。定义
在顶级 ``key`` 语句中的密钥可以用于所有视图中。想要用于
:any:`controls` 语句中的密钥必须定义在顶级中。
...
``key-store`` 语句定义如何存储DNSSEC密钥。

有一个内置密钥存储，名为 ``key-directory`` 。使用
``key-store "key-directory"`` 配置密钥等同于使用 ``key-directory`` 。

在一个 :any:`key-store` 语句中可以指定下列选项:

.. directory

   ``directory`` 指定这个密钥的密钥文件应当存放的位置。这类似于使用区的
   ``key-directory`` 。

.. namedconf:statement:: pkcs11-uri
   :tags: dnssec, pkcs11

   ``uri`` 是一个指定PKCS#11 URI Scheme（在 :rfc:`7512` 中定义）的字符串。
   如果设置， ``named`` 将会试图在对应的PKCS#11令牌中创建密钥。这要求BIND
   构建是带有OpenSSL 3，并且配置了一个PKCS#11提供者。
...
:any:`parental-agents` 列表允许一个通用的父区代理集合能够很容易地被用于
多个主区和辅区。“父区代理”是一个受信任的DNS服务器，它被查询以检查一个给
定区的DS记录是否保存最新。
...
.. namedconf:statement:: dnstap
   ...
   更多关于 :any:`dnstap` 的信息，参见 https://dnstap.info。
...
.. namedconf:statement:: qname-minimization
   ...
   当这个选项被设置为 ``strict`` 时，
   BIND将遵循QNAME最小化算法，如同在 :rfc:`7816` 中所描述。
   
   将这个选项设置为 ``relaxed`` 将使BIND回退到普通（非最小化）查询模
   式，即当它收到NXDOMAIN或其它不可预期的响应（如，SERVFAIL，不正确的
   区截断，REFUSED）时成为一个最小化请求。
   
   在 ``relaxed`` 模式， ``named`` 顺着树向下进行 ``<domain>`` 的NS查
   询。
...
.. namedconf:statement:: bindkeys-file
   ...
   更详细的内容参见对 :any:`dnssec-validation` 的讨论。这是用于服务器测试的。
...
.. namedconf:statement:: resolver-use-dns64
   :tags: server
   :short: 指定在发送查询时是否应用DNS64映射。

   如果 :any:`resolver-use-dns64` 被设置为 ``yes`` ，由 :any:`dns64` 选项
   所指定的从IPv4到IPv6的地址转换将被应用到递归查询将要发送到的IPv4服务器
   地址。这就允许一个服务器执行经由NAT64连接的查找；将要发送到IPv4的查询
   将被替代为发送到映射的IPv6地址。缺省是 ``no`` 。
...
.. namedconf:statement:: dnssec-loadkeys-interval
   ...
   当一个区配置了 ``dnssec-policy;`` ，必须定期检查其密钥仓库，
   以查看一次密钥轮转的下一步是否到期。
   :any:`dnssec-loadkeys-interval` 选项设置密钥仓库的检查间隔，以分钟
   计，以防无法计算下一次密钥事件（例如，因为一条DS记录需要发布）。
...
.. namedconf:statement:: max-zone-ttl
   ...
   配置了 :namedconf:ref:`dnssec-policy` 的区，在
   :namedconf:ref:`options` ， :namedconf:ref:`view` 和
   :namedconf:ref:`zone` 块中使用这个选项都是一个致命错误。
   在未配置 :namedconf:ref:`dnssec-policy` 的区，这个选项已被废弃，并且
   在未来的版本中将会被禁用而无法使用。
...
.. namedconf:statement:: require-cookie
   :tags: query
   :short: 控制是否接受不带服务器cookie的响应。

   ``require-cookie`` 子句可以用于指示远端服务器明确支持DNS COOKIE。
   将该选项设置为 ``yes`` 使得 ``named`` 在从远端服务器收到一个不带
   DNS COOKIE的UDP响应时，或者收到带有DNS COOKIE的UDP响应，却不是
   之前从本服务器发送的时候，总是通过TCP重发请求。这阻止 ``named``
   还未确定远端服务器是否支持DNS COOKIE时，在没有经过TCP重试前接受伪造的答复。
   
   将该选项设置为 ``no`` （缺省值）使得 ``named`` 依赖对DNS COOKIE
   支持的自动检查来决定合适经由TCP重试一个请求。
  
   .. note::
      如果一个UDP响应使用TSIG签名，即使 ``require-cookie`` 被设置为
      ``yes`` 且响应不包含DNS COOKIE， ``named`` 也会接受它。

   ``send-cookie`` 子句决定本地服务器是否在发给其它服务器的请求中添加
   一个COOKIE EDNS选项。这会覆盖view或option级别的 ``send-cookie`` 设置。
   :iscman:`named` 可以确定远端服务器不支持COOKIE，并且不在请求中添加
   COOKIE EDNS选项。
...
.. namedconf:statement:: stale-answer-client-timeout
   ...
   最小值 ``0`` ，导致立即返回一个缓存的（陈旧的）RRset，如果后者可用，
   同时仍然尝试刷新缓存中的数据。

   当这个选项开启时，在当前版本的BIND 9中唯一支持的值为 ``0`` 。非零值
   会生成一个警告信息，并被当成 ``0`` 。
...
.. namedconf:statement:: cookie-algorithm
   ...
   这个设置用于生成服务器cookie的算法。缺省是“siphash24”，这是仅存支持
   的选项，因为先前支持的“aes”选项已被删除。
...
.. namedconf:statement:: dnssec-validation
   ...
   密钥管理来保持更新。使用带有一个 ``static-key`` 锚类型的
   :any:`trust-anchors` 语句（或使用废弃的 :any:`trusted-keys` 语句）为根区
   添加一个显式静态密钥已不支持 ``auto`` 设置，会被当做一个配置错误。

   如果设置为 ``yes`` ，DNSSEC验证被开启，但是必须使用一个
   :any:`trust-anchors` 语句（或者 :any:`managed-keys` 或
   :any:`trusted-keys` 语句，这两个都已被废弃）手工配置一个信任锚；如
   果没有配置 :any:`trust-anchors` ，就是一个配置错误。如果
   :any:`trust-anchors` 不包含一个有效的根密钥，将不会对未被任何已配置
   的信任锚所覆盖的名字进行验证。

   如果设置为 ``no`` ，DNSSEC验证将被关闭。（注意：即使
   :any:`dnssec-validation` 被关闭，解析器仍然在其发出的查询中设置DO位，
   以指示它能接受DNSSEC响应。）

   缺省为auto，除非BIND使用 ``configure --disable-auto-validation`` 构
   建，在此情况下缺省为 ``yes`` 。

   缺省的根信任锚被编译进了 :iscman:`named` ，即发布日期的版本。如果根
   密钥发生变化，正在运行的BIND服务器会检测到这个变化并平滑地轮转到新密钥，
   但是新安装的服务器将不能启动验证，所以BIND必须升级到一个更新的版本。
...
.. namedconf:statement:: check-integrity
   ...
   使用 :iscman:`named-checkzone` ）。不在授权点的DS记录将被拒绝。
   缺省是 ``yes`` 。
...
.. namedconf:statement:: check-svcb
   :tags: zone
   :short: 指定是否对SVCB记录执行额外的检查。

   如果是 ``yes`` ，就对SVCB记录进行检查，要求后者以可选的 ``_<port>``
   标记为前缀的 ``_dns`` 标记（例如， ``_443._dns.ns1.example`` ）开始，
   有一个 ``alpn`` 参数，并且当 ``alpn`` 指示应当存在一个 ``dohpath``
   参数时，就有一个 ``dohpath`` 参数。缺省是 ``yes`` 。
...
.. namedconf:statement:: forwarders
   ...
   地址可以附带一个可选的端口号和一个TLS传输，还可以为整个列表设置一个缺省的端
   口号和一个TLS传输。

   如果指定的一个TLS配置，当 :iscman:`named` 连接到特定的IP地址时，它将使用
   DNS-over-TLS(DoT)连接，并使用由 :any:`tls` 语句所指向的TLS配置。
...
.. namedconf:statement:: allow-proxy
   :tags: server
   :short: 为允许发送PROXYv2头部的客户端地址定义一个 :any:`address_match_list` 。

   缺省的 :any:`address_match_list` 是 `none` ，这意味着出于安全考虑，
   缺省情况下不允许任何客户端执行此操作，因为PROXYv2协议提供了一种简便
   的方法来伪造源地址和目的地址。

   这个 :any:`address_match_list` 主要用于存放允许向BIND发送PROXYv2头部
   的代理的地址和子网。大多数情况，我们不推荐将这个
   :any:`address_match_list` 设置得太宽松，特别是将其设置为 `any` ，尤其
   在PROXYv2头部可以在公共可用的网络接口被接受的情况下。

   当使用PROXYv2头部时，指定的选项是唯一匹配真实对端地址的选项。大多数
   与对端地址协同工作的选项都是从PROXYv2头部中提取的。

   其它请参见： :namedconf:ref:`allow-proxy-on`

.. namedconf:statement:: allow-proxy-on
   :tags: server
   :short: 为允许发送PROXYv2头部的接口地址定义一个 :any:`address_match_list` 。该选项主要用于多接口配置。

   缺省的 :any:`address_match_list` 是 `any` ，意味着允许从任何接口
   接受PROXYv2。

   当你需要精确控制哪个接口允许PROXYv2时非常有用，因为这是唯一能够在
   使用PROXYv2头部时匹配真实接口地址的选项。大多数与接口地址协同工作
   的选项都是PROXYv2头部中提取的。

   你可能首先需要设置 :namedconf:ref:`allow-proxy` 。
...
.. namedconf:statement:: allow-transfer
   ...
   如果未指定 :any:`allow-transfer` ，缺省是 ``none`` ；对外区传送被关闭。
...
.. namedconf:statement:: no-case-compress
   ...
   如果未定义，ACL缺省为 ``none`` ：大小写敏感的压缩将用于所有客户端。
...
.. namedconf:statement:: listen-on-v6
   ...
   号，PROXYv2支持开关，TLS配置标识符和/或HTTP配置标识符，以及一个
   ...
   如果设定，PROXYv2支持开关 ``proxy`` 允许开启对PROXYv2协议的支持。
   PROXYv2协议提供了跨多层NAT或TCP/UDP代理将连接信息，如客户端的源地址、
   目的地址和端口，传递给后端服务器的手段。 
   
   PROXYv2协议所传递的地址将被使用，而不在使用操作系统所提供的对端和
   接口地址。

   ``proxy`` 开关可以有如下值：

   * ``plain`` - 接受普通的PROXYv2头部。对于不使用加密传输而言，这是
     唯一有效的选项。对使用加密传输，它指示BIND在TLS握手之前将PROXYv2
     头部以不加密的形式发送出去。在这种情况，只有PROXYv2头部不会加密。
   * ``encrypted`` - 接受加密的PROXYv2头部。在使用加密传输时，它指示
     BIND在TLS握手之后立即以加密方式发送PROXYv2头部。这个选项仅在使用
     加密传输时有效。

   你必须查询你的代理前端软件文档以确定你需要用到哪些值。如果不确定，
   为加密传输，特别是为DNS-over-HTTPS(DoH)，使用 ``plain`` ，但DNS专用
   软件很可能需要 ``encrypted`` 。

   应当注意，当一个监听器上开启了PROXYv2，它就失去了接受普通的唯一
   PROXYv2头部关联的DNS查询的能力。

   在某些情况，PROXYv2头部可能不包含有用的源或目的地址。特别是，这在
   头部使用 ``LOCAL`` 命令或者头部使用未指定或BIND不支持的地址类型时
   会发生。否则，BIND会接受这种头部，并在此情况使用真实的端点地址。

   PROXYv2协议被设计为可扩展的，并能携带类型-长度-值
   (type-length-values,TLVs)形式的附加信息。在协议规范中定义了多个类型，
   对其中的一些，我们做一定程度的验证，以检测和拒绝错误格式或者手工构
   造头部。除此之外，在收到这些附加数据，当前并不会被BIND用作其它任何
   目的。

   缺省时，即使一个 :any:`listen-on` 语句中开启了对协议的支持，也不
   允许客户端发送包含PROXYv2头部的查询。如果你对开启PROXYv2协议支持
   有兴趣，你还可以参考一下 :namedconf:ref:`allow-proxy` 和
   :namedconf:ref:`allow-proxy-on` 选项以调整对应的ACL。
...
.. namedconf:statement:: max-records-per-type
   :tags: server
   :short: 设置一个RRset中可以存储记录的最大数量

   这个设置在一个数据库中，一个RRset中可以存储的资源记录的最大数量。当
   在 :namedconf:ref:`options` 或 :namedconf:ref:`view` 中配置了这个，
   它控制缓存数据库；它也为区数据库设置缺省值，后者可以在
   :namedconf:ref:`zone` 级设置来覆盖。

   如果设置为一个正值，任何试图缓存或添加超过指定数量记录到一个区的一个
   RRset中都将导致失败。如果设置为0，RRset的大小没有上限。缺省值是100。

.. namedconf:statement:: max-types-per-name
   :tags: server
   :short: 设置一个属主名下可以存储的RR类型的最大数量

   这个设置在一个数据库中，单独一个属主名下可以存储的资源记录类型的最大
   数量。当在 :namedconf:ref:`options` 或 :namedconf:ref:`view` 中配置了
   这个，它控制缓存数据库并设置区数据库的缺省值，后者可以在
   :namedconf:ref:`zone` 级设置来覆盖。

   一个RR类型和它对应的签名被计算为两个类型。例如，一个包含A和AAAA记录的
   节点由四个类型：A，RRSIG(A)，AAAA和RRSIG(AAAA)。

   区和缓存数据库的行为有一点点不同：

   在一个区中，如果 :any:`max-types-per-name` 设置为一个正数，任何试图向
   一个已经含有指定数量类型的名字中添加新的资源记录集都会失败。

   在一个缓存中，如果 :any:`max-types-per-name` 设置为一个正数，试图向一
   个已经含有指定数量类型的名字中添加新的资源记录集会暂时成功，这样就可以
   回答查询。然而，新增的RRset将会立即被清除。

   某些高优先级类型，包含SOA，CNAME，DNSKEY及其对应的签名，总是被缓存的。
   如果 :any:`max-types-per-name` 被设置为太小的值，它可能被忽略，以允许
   高优先级类型被缓存。

   当 :any:`max-types-per-name` 被设置为0，RR类型的数量没有上限。缺省值是
   100。
...
.. namedconf:statement:: max-validations-per-fetch
   :tags: server
   :short: 设置在单次获取中可以发生的DNSSEC验证的最大数量。

   这是一个 **试验性的** 设置，设置在单次解析器获取中可以发生的DNSSEC
   验证的最大数量。缺省是16。

.. namedconf:statement:: max-validation-failures-per-fetch
   :tags: server
   :short: 设置在单次获取中可以发生的DNSSEC验证失败的最大数量。
...
.. namedconf:statement:: max-cache-size
   ...
   .. note::
       ...
       对于最大内存为100 MB或更低的情况， :any:`max-cache-size` 不能
       可靠地工作。
...
.. namedconf:statement:: sig0checks-quota
   :tags: server
   :short: 指定服务器可以处理的并发SIG(0)签名检查的最大数量。

   这是服务器能够接受的同时发生的SIG(0)签名消息的最大数量。如果达到
   限制， :iscman:`named` 就以状态码REFUSED答复。值 ``0`` 关闭限额。
   缺省为 ``1`` 。 

.. namedconf:statement:: sig0checks-quota-exempt
   :tags: server
   :short: 从SIG(0)签名检查限额中排查特定的客户端或客户端组。

   可以从SIG(0)签名检查限额中排某些DNS客户端，通过在
   :any:`sig0checks-quota-exempt` 子句中使用其IP和/或网络地址。缺省值
   是一个空表。
...
.. namedconf:statement:: notify-delay
   ...
   这个设置在发送一个区的NOTIFY消息集之间的延迟，单位为秒。无论何时发
   送一个区的NOTIFY消息，都会为这个时间段设置一个计时器。如果该区在计
   时器到期之前又被更新，更新的NOTIFY将被推迟。缺省是5秒。
...
.. namedconf:statement:: prefetch
   ...
   当收到一个有缓存且即将过期的请求， :iscman:`named` 可以立即从权威服
   务器刷新数据，确保缓存总有答复可用。

   :any:`prefetch` 指定"触发"TTL值，在这个值将进行当前请求的预取：当在
   查询过程中遇到一个小于或等于TTL值的缓存记录，它将被刷新。有效的触发
   TTL值是1到10秒。超过10秒的值将被静默地减到10。将一个触发TTL设置为零
   将关闭预取功能。缺省的触发TTL是 ``2`` 。

   一个可选的第二参数指定"合格" TTL：对于一个适合预取的记录可接受的最
   小 **初始** TTL值。合格TTL必须至少比触发TTL大六秒：如果不是这样，
   :iscman:`named` 将静默地向大调整。缺省的合格TTL是 ``9`` 。

.. namedconf:statement:: v6-bias
   :tags: server, query
   :short: 指示IPv6名字服务器优先的毫秒数。

   在决定要试探的下一个名字服务器时，这个指示IPv6的名字服务器会优先的
   毫秒数。缺省是 ``50`` 毫秒。
...
.. namedconf:statement:: dnsrps-library
   :tags: server, security
   :short: 打开DNS响应策略服务(DNS Response Policy Service, DNSRPS)接口。

   这个选项指定DNSRPS提供者库的路径。典型情况，这个库是在使用
   ``configure --enable-dnsrps`` 构建时被检测到，不需要在
   ``named.conf`` 中指定；出于测试目的，这个选项提供一个覆盖缺省库
   的选择。
...
.. namedconf:statement:: dnsrps-options
   ...
   DNSRPS提供者库被传递一个由 :any:`dnsrps-options` 文本组成
...
响应策略区可以被配置成对被响应策略修改的响应设置一个扩展的DNS错误(EDE)码。
...
.. namedconf:statement:: rate-limit
   ...
   每个同一响应和客户端的组合有一个概念上的“帐户”，会每秒挣得一个特定
   数目的信用值。一个预期的响应将会从其帐户上减一。当帐户是负值时，
   响应将被丢弃或者被截断。
...
息，必须正确地限制连接请求的源，仅受信任方才能访问统计通道。

搜集统计通道公开的数据会锁定 :iscman:`named` 中的各个子系统，如果请求
太频繁，可能会降低查询处理的速度。

仅当普通用户可以利用统计通道中的问题绕过访问控制表时，该问题才会被认为
是一个安全问题。换句话说，如果可以通过使用一个安全配置来避免，统计通道
中可能用于访问不可用的信息或导致 :iscman:`named` 崩溃之外的任何问题都
不被认为是一个安全问题。
...
http://127.0.0.1:8888/xml/v3/xfrins（入向区传送统计），
http://127.0.0.1:8888/xml/v3/net（网络状态和套接字统计），
http://127.0.0.1:8888/xml/v3/mem（内存管理统计），和
...
http://127.0.0.1:8888/json/v1/xfrins（入向区传送统计），
http://127.0.0.1:8888/json/v1/net（网络状态和套接字统计），
http://127.0.0.1:8888/json/v1/mem（内存管理统计），和
...
.. namedconf:statement:: cipher-suites
   :tags: security
   :short: 指定了仅适用于TLSv1.3的加密套件列表，以优先级顺序排列。

    加密套件列表定义了允许使用的加密套件，例如
    ``TLS_AES_256_GCM_SHA384:TLS_CHACHA20_POLY1305_SHA256：TLS_AES_128_GCM_SHA256`` 。
    该字符串必须按照OpenSSL文档中规定的规则来构建（详情请参阅
    https://www.openssl.org/docs/man1.1.1/man1/ciphers.html 
    中的“TLS v1.3 密码套件”一节）。
   
.. namedconf:statement:: ciphers
   :tags: security
   :short: 指定仅适用于TLSv1.2的密码列表，以优先级顺序排列。
...
.. namedconf:statement:: session-tickets
   ...
   :short: 通过TLS会话单据启用或禁用会话恢复。
    ...
    通过TLS会话单据启用或禁用会话恢复，定义在 :rfc:`5077` 中。在需要前向保密
...
使用 ``ephemeral`` 配置的主要动机是帮助测试，因为受信任的证书颁发机构
不会颁发与此配置相关联的证书。因此，在验证TLS证书时，这些证书永远不会
被任何客户端所信任。它们对流量提供了加密，但没有对传输通道提供认证。
在一个可控环境中的部署情况这就足够了。

应该注意的是，在重新加载配置时， ``ephemeral`` TLS密钥和证书会重新创建，
所有TLS证书和密钥，以及相关的数据，都会从磁盘重新加载。在这种情况，与
TLS相关联的监听套接字仍然不变。

请牢记做重配置可能导致BIND处理进入的客户端包的能力有一小段中断。中断的
时长与环境和配置相关。何时做重配置的一个好的例子是当TLS密钥和证书被更
新到磁盘上时。
...
选项时，就使用此模式。当TLS不可用时， :rfc:`9103` 允许可选的回退到明文DNS。
...
为根区设置一个 ``initial-key`` 。此初始化密钥已内置在 :iscman:`named`
中，并且自发布日期起一直有效。当根区密钥发生变化时，运行中的服务器会
检测到该变化并轮转到新的密钥，但新安装并首次运行的服务器需要是使用了
较新版本的 BIND（且该版本已包含当前密钥）的服务器。
...
:any:`dnssec-policy` 语句要求设置动态DNS，或者开启为
:any:`inline-signing` （这对DNSSEC区是缺省的）。

如果开启了 :any:`inline-signing` ，这意味着区的一个签名版本被独立维护
并被写到磁盘上的一个不同文件中（区文件名加上 ``.signed`` 扩展）。

如果关闭了 :any:`inline-signing` ，需要使用 :any:`update-policy`
或者 :any:`allow-update` 配置区。在此情况，DNSSEC记录被写到由
最初的区的 :any:`file` 中设置的文件中。
...
.. namedconf:statement:: cdnskey
   :tags: dnssec
   :short: 指定在KSK轮转时，是否发布一个CDNSKEY记录。

   当设置为缺省值 ``yes`` 时，在KSK轮转时，发布一个CDNSKEY记录，
   这时后继密钥的DS可能被提交给父区。

.. namedconf:statement:: cds-digest-types
   :tags: dnssec
   :short: 指定用于CDS资源记录的摘要类型。

   这指定生成CDS资源记录时的摘要类型。缺省仅是SHA-256。
...
:any:`inline-signing`
   :tags: dnssec
   :short: 指定BIND9是否维护一个区的单独签名版本。

   如果为 ``yes`` ，BIND 9维护一个区的单独签名版本。未签名的区通过
   区传送进入或者从磁盘装载，签名版本很可能以不同的序列号对外服务。
   签名版本的区存储在一个以区的文件名（在 :any:`file` 中设置）加上
   一个 ``.signed`` 扩展命名的文件中。
...
.. keys
   ...
   这里是在一个 ``keys`` 列表中可能出现的一些条目的一个例子（仅作
   说明用途）。
   ...
   一个可选的第二个符号决定密钥的存储位置。两个可用的选项是
   ``key-store <string>`` 和 ``key-directory`` 。
    
   当使用 ``key-store`` 时，被引用的 :any:`key-store` 描述密钥应
   如何存放。这可能是一个文件，或者它也可能在一个PKCS#11符号中。

   当使用 ``key-directory`` ，密钥被存储在区的 :any:`key-directory`
   配置中。这也是缺省值。

   ``lifetime`` 参数指定一个密钥在轮转前使用多长时间。为方便起见，
   可以使用TTL风格的时间单位后缀指定密钥生命周期。也接受ISO 8601
   持续时间格式。

   在上面的例子中，第一个密钥有无限的生存期，第二个密钥可以使用30
   天，第三个密钥则具有一个相当特别地生存期，6个月，12小时，3分又
   15秒。一个为0秒的生存期与 ``unlimited`` 相同。
   ...
   ZSK，它是 :any:`max-zone-ttl` ， :any:`retire-safety` ，
   :any:`zone-propagation-delay` 与签名延迟（ :any:`signatures-validity`
   减 :any:`signatures-refresh` ）之和，对于KSK和CSK，它是
...
.. note:: 修改 ``key-directory`` 或 ``key-store`` 时，BIND不能发现
   当前的密钥文件。在配置文件中修改已使用的路径之前，确保将密钥文件
   拷贝到新的目录。修改一个内置的策略也是一样，例如 ``insecure`` 。
   在这种特定情况，应当将区的现存密钥文件从新的配置中移动到区的
   ``key-directory`` 目录中。
...
.. namedconf:statement:: signatures-jitter
   :tags: dnssec
   :short: 为签名的过期时间指定一个范围。

    为阻止所有签名在同一时刻过期，BIND 9可能对每个单独签名的有效期
    范围做出改变。一个新生成的签名的有效期范围是在
    :any:`signatures-validity` （最大值）和 :any:`signatures-validity`
    减去 :any:`signatures-jitter` （最小值）之间。缺省的抖动值是12
    小时并且配置的值必须小于 :any:`signatures-validity` 和
...
.. namedconf:statement:: nsec3param
   ...
   :short: 指定使用NSEC替代NSEC，并设置NSEC3参数。
    ...
    .. warning::
       不要使用额外的 :term:`iterations <Iterations>` ， :term:`salt <Salt>` 和
       :term:`opt-out <Opt-out>` ，除非完全理解其含义。一个较大循环次数导致互操
       作问题并使服务器面临CPU耗尽的DoS攻击。参见 :rfc:`9276` 。
...
.. namedconf:statement:: checkds
   :tags: dnssec
   :short: 控制是否将 ``DS`` 查询发送给父区代理。

   如果设置为 ``yes`` ，当KSK轮转进行时会发送DS查询。查询会发给在父区的NS
   记录中所列出的服务器。如果没有为区配置 :any:`parental-agents` ，这是
   缺省值。

   如果设置为 ``explicit`` ，DS查询仅发送给使用 :any:`parental-agents` 显式
   列出的服务器。如果配置了父区代理，这是缺省值。

   如果设置为 ``no`` ，不发送DS查询。用户应当手动运行
   :option:`rndc dnssec -checkds <rndc dnssec>` ，并配以合适的参数，以
   发出信号表示特定的DS记录已被发布和/或撤销。
...
.. namedconf:statement:: type
   ...
   :any:`forward <type forward>` 或 :any:`redirect <type redirect>` 。
...
.. namedconf:statement:: type mirror
   ...
   列表（参见 :any:`primaries` 获取详细信息），并且要将指
...
.. namedconf:statement:: type redirect
   ...
   由于重定向区不会直接被名字引用，它们不保存在普通主区和辅区的区查找
   表中。因此，当前不可能使用要重新装载一个重定向区，使用
   :option:`rndc reload -redirect <rndc reload>` ，要重新传输一个作为
   辅区配置的重定向区，使用
   :option:`rndc retransfer -redirect <rndc retransfer>` 。当使用
   :option:`rndc reload` 而不指定一个区名时，重定向区将会随着其它区被
   重载。
...
.. namedconf:statement:: in-view
   ...
   使用多个视图时，配置在一个视图中的一个 :any:`type primary` 或
   :any:`type secondary` 区可以被随后的视图引用。这允许两个视图使用同
   一个区而无须加载多次。这通过一条 :any:`zone` 语句，以及一个
   :any:`in-view` 选项指定区所定义的视图来配置。一条包含
   :any:`in-view` 的 :any:`zone` 语句不需要指定类型，因为那是在其它视
   图中的区定义的一部份。
...
:any:`dnssec-loadkeys-interval`
   参见 :namedconf:ref:`options` 中对 :any:`dnssec-loadkeys-interval` 的描述。

:any:`dnssec-update-mode`
   参见 :namedconf:ref:`options` 中对 :any:`dnssec-update-mode` 的描述。
...
:any:`max-ixfr-ratio`
   参见 :namedconf:ref:`options` 中对 :any:`max-ixfr-ratio` 的描述。
...
:any:`zone-statistics`
   参见 :namedconf:ref:`options` 中对 :any:`zone-statistics` 的描述。
...
:any:`key-directory`
   参见 :namedconf:ref:`options` 中对 :any:`key-directory` 的描述。

:any:`serial-update-method`
   参见 :namedconf:ref:`options` 中对 :any:`serial-update-method` 的描述。

.. namedconf:statement:: inline-signing
   :tags: dnssec, zone
   :short: 指定BIND 9是否维护一个区的单独的签名版本。

   内联签名的使用由区的 :any:`dnssec-policy` 决定。如果在 :any:`zone`
   中， :any:`inline-signing` 被显式设置为 ``yes`` 或者 ``no`` ，它就覆盖
   :any:`dnssec-policy` 的任何值。
...
:any:`max-zone-ttl`
   参见 :namedconf:ref:`options` 中对 :any:`max-zone-ttl` 的描述。
...
在一个 :any:`inline-signing` 被关闭的DNSSEC区的情况，DNSSEC记录也写到区的文件名。
...
.. namedconf:statement:: update-policy
   ...
   个身份所更新。身份是由密钥决定的，这个密钥使用TSIG或SIG(0)签名更
   ...
   ``identity`` 字段必须设置成一个完整域名。在大多数情况，它表示成
   必须被用于签名更新请求的TSIG或SIG(0)密钥的名字。如果指定的名字是
...
Incoming Zone Transfers
   ...
   本节描述了关于正在进行的入向区传送的、能在HTML表中看见的信息。
   它列出了含义、单位和每列值的可能范围，以及为JSON和XML输出格式
   的密钥/属性/元素名（在括号中）。

   ``Zone Name`` (``name``)
      文本串。这是正在被传送的区的名字，由本服务器上的 :any:`zone`
      声明所指定。

   ``Zone Type`` (``type``)
      文本串。这是正在被传送的区的类型，由本服务器上的 ``zone``
      声明所指定。可能的值为： ``secondary`` ， ``stub`` ，
      ``redirect`` ， ``mirror`` 。

   ``Local Serial`` (``serial``)
      32位无符号整数。这是正在被传送的区的当前（旧）序列号。它来自
      于当前服务器所持有的SOA记录。

   ``Remote Serial`` (``remoteserial``)
      32位无符号整数。这是正在被传送的区的新序列号。它来自于
      正在从其进行区传送的主服务器所持有的SOA记录。

   ``IXFR`` (``ixfr``)
      布尔值。这说明区传送是增量（使用IXFR）或全量（使用AXFR）。可
      能的值是： ``Yes`` ， ``No`` 。

   ``State`` (``state``)
      文本串。这是区的传输状态。可能的值及其含义为：

         ``Needs Refresh``
             区需要一次刷新，但是进程还未开始，刷新可能有不同原因，如区
             的重试间隔。

         ``Pending``
	     区被标记为一次刷新，但是进程当前处于一个队列，不久将被启动，
             或者因为速率限制，参见 :any:`serial-query-rate` ，而处于等待
             状态。在进入该状态之前启动 ``Duration (s)`` 计时器。

         ``Refresh SOA``
	     发送一次刷新的SOA查询以获取区序列号，如果需要，就发起一次
             区传送。如果此步成功，将跳过 ``SOA Query`` 和 ``Got SOA`` 状态。
             否则，区传送过程仍然会被发起，而SOA请求可能会尝试复用与区传送
             一样的传输。在进入此状态之前，对每个尝试的（注意，在UDP模式，
             一次 "连接" 尝试可能有好几次重试）连接重新开始 ``Duration (s)``
             计时器。

         ``Deferred``
	     区正在被刷新，但是进程由于配额被延迟，参见 :any:`transfers-in`
             和 :any:`transfers-per-ns` 。在进入此状态前重新开始
             ``Duration (s)`` 计时器。

         ``SOA Query``
	     发送SOA查询以获取区的序列号，如果需要，随后就是一次区传送。在
             进入这个状态前， ``Duration (s)`` 计时器重新计时。

         ``Got SOA``
             收到一个先前步骤的SOA查询的答复，初始化一次传输。

         ``Zone Transfer Request``
	     等待区传送开始。在进入此状态之前重新开始 ``Duration (s)`` 计时器。

         ``First Data``
             等待区传送的第一个数据记录。

         ``Receiving IXFR Data``
	     为一次IXFR类型的增量区传送而接收数据。
   ...
   ``Additional Refresh Queued`` (``refreshqueued``)
      布尔值。这展示区已被标记为刷新。当区传送仍然为一个待处理状态
      （参见 ``State`` 列的描述），或者当区传送为正在运行状态，但是
      区又被标记为另一次刷新（例如，由于来自一个主服务器的 "notify"
      请求），这个会被设置为 ``Yes`` ，可能的值为： ``Yes``, ``No`` 。

   ``Local Address`` (``localaddr``)
      IP地址 - IPv4或IPv6，视情况而定，以及端口号。这显示区传送中，
      用于建立连接的源地址。
   ...
   ``SOA Transport`` (``soatransport``)
      文本串。这是用于SOA查询的传输协议。注意，在过程中，这个值可能
      潜在地变化。例如，当区传送为 ``Refresh SOA`` 状态时，正在进行的
      ``SOA Transport`` 查询可能显示 ``UDP`` 。如果查询失败或超时，
      可以使用另一种传输方式重试，也可在 "SOA before" 模式启动
      传输过程，其中将尝试与区传送同样传输方式的SOA查询。更多信息
      参见 ``State`` 字段的说明。可能的值为： ``UDP``, ``TCP``, ``TLS``, ``None`` 。

   ``Transport`` (``transport``)
      文本串。这是区传送的传输协议。可能的值为： ``TCP``, ``TLS`` 。

   ``TSIG Key Name`` (``tsigkeyname``)
      文本串。这是在 :any:`zone` 声明（如果有的话）中指定的用于此区的
      TSIG密钥的名字。

   ``Duration (s)`` (``duration``)
      64位无符号整数。这是以秒为单位的当前传送进程的主状态已经运行
      的时间。计时器在刷新SOA请求进入到队列后（在 ``Pending`` 之前）
      开始，然后在整个过程中它会重启动几次，以指示当前主状态的持续
      时间。参见对不同状态的描述，以了解计时器重新开始前的状态。

   ``Messages Received`` (``nmsg``)
      64位无符号整数。这是收到的DNS消息的数量。它不包括传输开销，
      如TCP ACK。

   ``Records Received`` (``nrecs``)
      64位无符号整数。这是已收到的单独资源记录的数量。例如，如果
      一个地址记录中有五个地址与同一名字相关联，就计算为五个资源记录。

   ``Bytes Received`` (``nbytes``)
      64位无符号整数。这是DNS数据用到的字节数。它不包含传输头部。

   .. note::
      依赖于传输的当前状态，一些值可能为空或设置为 ``-`` （意思
      是 "不可用"）。此外，对于JSON输出格式而言，相应的关键字可能会
      缺失，或者值可能会被设置为 ``NULL`` 。例如，在接收到第一组数据
      之前，无法确定传输是使用AXFR还是IXFR（请参阅 ``State`` 列的
      说明）。
...
兼容。参见 :namedconf:ref:`options` 中对 :any:`zone-statistics` 更详细的描述。
...
``QryDropped``
    这指示被服务器作为已配置限额的结果而丢弃的递归查询数量。这些
    限额包括 :any:`fetches-per-zone` ， :any:`fetches-per-server` ，
    :any:`clients-per-query` 和 :any:`max-clients-per-query` 选项，
    以及 :any:`rate-limit` 选项。这对应于以前BIND 9版本中的
    ``dropped`` 计数器。
...
``BucketSize``
    这指示解析器内部桶（一个静态数字）的数量。

``REFUSED``
    这指示收到的REFUSED响应的数量。

``ClientCookieOut``
    这指示仅带有客户端发送cookie的COOKIE数量。

``ServerCookieOut``
    这指示带有客户端和服务器发送cookie的COOKIE数量。
...
``BadEDNSVersion``
    这指示已收到的错误EDNS版本答复的数量。

``BadCookieRcode``
    这指示已收到的错误cookie响应码答复的数量。
...
``ClientQuota``
    这指示因每个客户端查询限额而溢出的查询的数量。

``NextItem``
    这指示当收到一个不正确响应时等待下一个响应的数量。

``Priming``
    这指示由解析器执行的priming提取的数量。
...
``UDP6`` （UDP/IPv6）， ``TCP4`` （TCP/IPv4），和 ``TCP6`` （TCP/IPv6）。
...
``<TYPE>AcceptFail``
    这指示接受进入连接请求失败的数量。这个计数器不适用于 ``UDP`` 类型。

``<TYPE>Accept``
    这指示成功接受进入连接请求的数量。这个计数器不适用于 ``UDP`` 类型。
