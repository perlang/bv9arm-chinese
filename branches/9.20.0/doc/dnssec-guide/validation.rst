...
完成这个目标，你仅仅需要在你的配置文件的 :namedconf:ref:`options` 部份
增加一行：
...
重启 :iscman:`named` 或执行 :option:`rndc reconfig` ，然后你的递归服务
器现在就能够愉快地验证每条 DNS 响应了。如果这对你不起作用，你可能还需
要调整一些其它网络相关的配置。参考 :ref:`network_requirements` 以确保
你的网络已经为 DNSSEC做好了准备。
...
安装 :iscman:`dig` ，可以从 ISC 的 `网站 <https://www.isc.org/download>`__
下载并安装。
...
与早期版本的BIND不同，当前版本的BIND总是请求DNSSEC记录（通过在对上游服
务器的查询中设置 ``do`` 位），不管DNSSEC设置如何。然而，禁用验证后，不
会检查返回的签名。这可以通过显式禁用DNSSEC验证来看到。要做到这一点，在
配置文件的 :namedconf:ref:`options` 部分添加一行
``dnssec-validation no;`` ，例如：
...
这个“auto”行使用 :any:`managed-keys` 特性开启自动DNSSEC信任锚配置，此时
不需要手动配置密钥。 :any:`dnssec-validation` 选项有三种可能的选择：
...
-  *auto* ：启用DNSSEC验证，并使用DNS根区的默认信任锚（包含在BIND 9中
   ）。这是缺省值；如果配置文件中没有 :any:`dnssec-validation` 行，
   BIND会自动执行此操作。

让我们讨论一下 *yes* 和 *auto* 之间的区别。如果设置为 *yes* ，则必须使
用配置文件中的 :any:`trust-anchors` 语句(可以带 ``static-key`` 或
...
在使用 *yes* 的情况时，请注意如果 :any:`trust-anchors` 不包含一个有效的
根密钥，就不会对没有被任何已配置的信任锚所覆盖的名字进行验证。
...
将 :any:`dnssec-validation` 设置为 *auto* ，就没有什么可做的了。BIND已经包
含了一个根密钥的副本，并在根密钥发生变化时自动更新它。 [#]_ 它看起来像这样：
...
:any:`dnssec-validation` 被设置成 *yes* 而不是 *auto* ：
...
然后，从值得信任的源手工下载根密钥，并将其放入一条 :any:`trust-anchors` 语句中，
如下所示：
...
这个 :any:`trust-anchors` 语句看起来类似于上述的内置版本，内置密钥有
``initial-key`` 修饰符，而在配置文件的语句中，它被 ``static-key`` 所取
代。这两者之间有一个重要的区别：用 ``static-key`` 定义的密钥在从配置文
件中删除之前始终是可信的。而带有 ``initial-key`` 修饰符的密钥只受信任
一次：直到它加载被管理密钥数据库并启动密钥维护过程。此后，BIND使用被管
理密钥数据库 (``managed-keys.bind.jnl``)作为密钥信息的来源。
...
从那以后， `超过90% <https://ithi.research.icann.org/graph-m7.html>`__
