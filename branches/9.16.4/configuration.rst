...
``named-checkzone``
   ``named-checkzone`` 程序用来检查一个主文件的语法和一致性。
...
``rndc``
   ...
   ``key`` 关键字后跟一个密钥名，它可以是一个有效的域名，尽管它并不需要
   一个层次结构；因而，一个像 “ ``rndc_key`` ”这样的字符串也是一个有效
   的名字。 ``key`` 语句有两个子句： ``algorithm`` 和 ``secret`` 。
   配置分析器将接受任何字符串作为算法参数，当前只有字符串 ``hmac-md5`` ，
