...
.. option:: -t timeout
   ...
   本选项为 :program:`rndc` 设置空闲超时时间为 ``timeout`` 秒。缺省为
   60秒，最大可设置的值为86400秒（1天）。如果设为0，就不会超时。
...
.. option:: fetchlimit [view]
   ...
   这个命令转储当前作为 ``fetches-per-server`` 设置结果的响应率限制
   的服务器列表，以及当前作为 ``fetches-per-zone`` 设置结果的响应率
   限制的服务器列表。
...
.. option:: loadkeys [zone [class [view]]]
   ...
   这个命令要求使用 ``dnssec-policy`` 配置区，而且还要求区被配置为允许动态
...
.. option:: sign zone [class [view]]
   ...
   这个命令要求使用 ``dnssec-policy`` 配置区，还要求区被配置为允许
   动态DNS。（更详细情况参见BIND 9管理员参考手册中的“动态更新策略”。）
...
.. option:: thaw [zone [class [view]]]
   ...
   则，如果区被修改，会删除所有现存的日志文件。如果未指定区，重新加载会
   异步进行。
