...
:iscman:`named` 创建了“FEDCBA09”(盐长度8)作为新盐：
...
如果你的父区允许通过CDS/CDNSKEY管理DS记录，如同在 :rfc:`5155` 中所描述
的，你可以在区中添加CDS/CDNSKEY DELETE记录，以表示需要从父区删除对应的
DS记录。如果不清楚父区需要哪个格式，则应该同时发布CDS和CDNSKEY DELETE
记录。
...
的区能够平滑地迁移到未签名的状态。它还会在适当的时候为你发布CDS和
CDNSKEY DELETE记录。

当DS记录从父区中删除时，使用
:option:`rndc dnssec -checkds -key id withdrawn example.com <rndc dnssec>` 告诉 :iscman:`named`
DS已被删除，其余的DNSSEC记录将被及时删除。或者如果你配置了父区代理，
DNSSEC记录将会在BIND看到父区代理不再为这个区的DS资源记录集服务时被自动
地删掉。

一段时间后，区将恢复到传统的、不安全的DNS格式。可以通过检查所有DNSKEY和
RRSIG记录是否已从区中删除来验证。

然后可以从 :iscman:`named.conf` 中删除 ``dnssec-policy`` 行并重新装载区。区现
在已恢复到传统的、不安全的DNS格式。
