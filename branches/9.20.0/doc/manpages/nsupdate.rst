...
.. option:: -A tlscafile
   ...
   本选项指定认证中心（certificate authorities，CA）的证书文件（PEM格式），用以
   在使用DNS-over-TLS（DoT）时验证远端服务器的TLS证书，以实现严格或相互TLS。如
   果使用，它将会覆盖来自全局证书仓库的证书，而后者是开启 :option:`-S` 时缺省
   使用的。本选项不能与 :option:`-O` 组合使用，它是隐含带 :option:`-S` 的。
...
.. option:: -E tlscertfile
   ...
   本选项设置用于在使用DNS-over-TLS (DoT)传输到远端服务器时进行认证的证书文件。
   期望的证书链文件为PEM格式。本选项隐含 :option:`-S` ，并且只能与
   :option:`-K` 配合使用。
...
.. option:: -H tlshostname
   ...
   本选项使 :program:`nsupdate` 在远端服务器TLS证书验证时使用所提供的主机名。
   否则，使用DNS服务器的名字。本选项隐含 :option:`-S` 。
...
.. option:: -K tlskeyfile
   ...
   本选项设置用于在使用DNS-over-TLS (DoT)与远端服务器进行传输时用于认证加密的
   密钥文件。期望的私钥文件是PEM格式。本选项隐含 :option:`-S` ，并且只能与
   :option:`-E` 配合使用。
...
.. option:: -o
   ...
   本选项已被废弃。之前，它开启一个用于Windows 2000的非标准兼容的GSS-TSIG变体。
   由于此操作系统已过生命周期终点，本选项现已被当做 :option:`-g` 的同义词。

.. option:: -O

   本选项开启Opportunistic TLS。如果使用，将不会验证远程对端的TLS证书。
   仅应处于调试目的使用本选项，不推荐将其用于生产环境。本选项不能与 :option:`-A`
   共同使用，且它隐含 :option:`-S` 。
...
.. option:: -S
   ...
   本选项指示在查询由输入文件中 ``server servername port`` 语法中指定的
   名字服务器，并且通过一次SOA请求发现主服务器时是否使用DNS-over-TLS (DoT)。
   如果使用了 :option:`-K` 和 :option:`-E` 选项，使用指定的TLS客户端证书
   和私钥对进行认证（共同TLS）。本选项隐含 :option:`-v` 。

.. option:: -t timeout

   本选项设置一个更新请求在其被中断之前可以持续的最大时间。缺省是300秒。
   如果为0，就对TCP模式关掉超时。对于UDP模式，选项 :option:`-u` 比本
   选项更优先，除非选项 :option:`-u` 被设置为0，在此情况，会从
   :option:`-t` 超时间隔和UDP重试次数计算间隔。对于UDP模式，超时不能被
   关掉，但在 :option:`-t` 和 :option:`-u` 都被设置为0时，它可以被向上
   取整为1秒。
...
``oldgsstsig``
   这个命令已被废弃并会在将来的版本中删除。之前，它使用Windows 2000版的
   GSS-TSIG对更新签名。现在它被当做 `gsstsig`` 的同义词看待。
...
``check-svbc [boolean]``
   这个命令开启或关闭在添加记录时的check-svcb处理。check-svcb对先决条件
   中的记录或者对要删除的记录没有影响。缺省时check-svcb处理为开启。如果
   check-svcb处理失败，记录不会添加到UPDATE消息中。

``lease time [keytime]``
  设置EDNS Update Lease(UL)选项的值为 ``time`` ，并且，作为选项，可以将
  密钥的租用时间设置为 ``keytime`` ，单位为秒。如果 ``time`` 为 ``none`` ，
  租用时间被清除。
