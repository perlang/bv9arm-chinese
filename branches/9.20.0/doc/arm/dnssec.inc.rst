...
:any:`inline-signing` 。在上面的例子中我们使用了后者，因为 ``default``
策略使用 :any:`inline-signing` 。
...
因为BIND不会验证响应。 ``checkds`` 选项使BIND显式地使用已配置的父区代理，
而不是通过查询父区NS记录来查找它们。
...
并服务于同样的区，如 :rfc:`8901` 所描述。BIND 9能够支持模式2，在其中，
每个提供者有自己的KSK和ZSK（或者CSK）。来自其它提供者的密钥可以经由动态
更新导入。因此，每个活跃的KSK必须有一个对应的DS记录在父区中。密钥轮转需
要协调，以便更新DS和DNSKEY资源记录集。
...
:doc:`dnssec-guide` 中的
:ref:`advanced_discussions_manual_signing` 以获得更
多信息。
