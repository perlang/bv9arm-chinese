...
BIND 9.20要求 ``libuv`` 1.34.0或更高版本，推荐使用 ``libuv`` >=1.40.0。
...
Userspace RCU库 ``liburcu`` (https://liburcu.org/) 用于无锁数据结构和并发
安全内存回收。

在Linux上，使用 ``libcap`` 库
(https://git.kernel.org/pub/scm/libs/libcap/libcap.git/)在用户空间下管理
进程性能，它可以通过 ``libcap-dev`` 或 ``libcap-devel`` 包安装在大多数
Linux系统上。
...
要提高性能，强烈建议使用 ``jemalloc`` 库(http://jemalloc.net/)。如果要使用，
要求4.0.0或更新的版本。
