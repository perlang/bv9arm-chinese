...
包括DNS的互联网协议族的规范文档是作为 `请求注释`_ 
(Request for Comments, RFC)系列的技术备忘的一部份被出版的。标准本身是由
`互联网工程任务组`_ (Internet Engineering Task Force, IETF)和
`互联网专家任务组`_ (Internet Engineering Steering Group, IESG)
IESG）所定义。RFC可以在 https://www.rfc-editor.org/ 查看。

在阅读RFC时，请注意 :rfc:`不是所有的RFC都是标准 <1796>`, 并且文档的有效
性随着时间而变化。每个RFC都需要在其它文档的上下文中被解释。

BIND 9力求严格遵守IETF标准。据我们所知，BIND 9遵守以下RFC，并在以下编号
的注释中列出了注意事项和例外情况。这些RFC中的许多是由ISC的员工或前员工
所撰写。这个列表是不完全的。

.. _`互联网专家任务组` : https://www.ietf.org/about/groups/iesg/
.. _`互联网工程任务组` : https://www.ietf.org/about/
.. _`请求注释` : https://www.ietf.org/standards/rfcs/
