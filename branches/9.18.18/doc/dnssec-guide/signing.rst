...
.. note::
   ...
   之前，如果一个区使用一条 :any:`dnssec-policy` 而没有设置动态DNS并且
   没有显式设置 :any:`inline-signing` ，BIND 9隐含使用内联签名。但是在
   操作者对其区开启或关闭动态DNS时，这会导致许多问题。因此，现在你必须
   显式配置它。
...
-  :iscman:`named` 配置文件中的 :any:`dnssec-policy` 语句描述了DNSSEC
   策略的所有方面，包括签名。

-  :any:`dnssec-policy` 语句要求区使用动态DNS，或者开启
   :any:`inline-signing` 。
