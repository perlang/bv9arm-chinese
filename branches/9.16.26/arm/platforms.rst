...
当前不同平台和BIND 9版本的支持状态可以在ISC的知识库找到：
...
-  需要来自编译器的对原子操作的支持，要么以内置操作的形式，要么是C11原
   子的形式，要么是Windows上 ``Interlocked`` 函数族的形式。

-  需要来自编译器的对线程本地存储的支持，要么是C11
   ``_Thread_local``/``thread_local`` 的形式，要么是 ``__thread`` GCC扩
   展，或者在Windows上的 ``__declspec(thread)`` MSVC扩展。
...
截止到2021年12月，BIND 9.16在下列系统上是完全支持和经过定期测试的：
...
除了Windows Server 2016之外，ISC没有对这些进行定期测试。
