...
:program:`dnssec-keygen` 为在 :rfc:`2535` 和 :rfc:`4034` 中定义的
DNSSEC（安全DNS）生成密钥。
...
.. option:: -a algorithm
   ...
   ECDSAP384SHA384，ED25519或ED448之一。
...
.. option:: -f flag
   ...
   本选项在KEY/DNSKEY记录的标志字段中设置特定的标志。仅能被识别的标志是
   ZSK（区签名密钥）、KSK（密钥签名密钥）和REVOKE。

   注意ZSK不是DNSKEY记录中的物理标志，它仅仅用于显式说明需要创建一个ZSK。
   结合使用 :option:`-f` 和 :option:`-k` 将导致生成只匹配本选项指定角色
   集的密钥。

.. option:: -F

   本选项开启FIPS（美国联邦信息处理标准）模式，如果底层加密库支持运行于
   FIPS模式。
