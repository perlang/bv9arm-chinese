...
.. option:: -a anchor-file
   ...
   本选项指定从中读取一个替代的DNSSEC根区信任锚点的文件。
   
   缺省时，与根区名字(`.`)不匹配的密钥会被忽略；如果期望一个替代的密钥名，
   可以使用 :option:`+root` 选项指定。

   注意：在读取信任锚点时， :program:`delv` 同等对待 ``trust-anchors`` ，
   ``initial-key`` 和 ``static-key`` 。即，对于一个被管理的密钥，受
   信任的是 **初始的** 密钥， :rfc:`5011` 密钥管理是不支持的。 :program:`delv`
   不会询问由 :iscman:`named` 维护的被管理密钥数据库。这意谓着，如果
   :program:`delv` 内置的缺省密钥被撤销， :program:`delv` 必须被更新到
   一个更新的版本以继续验证。
...
.. option:: +hint=FILE, +nohint
   ...
   本选项指定一个从其中装载根提示信息的文件名；这将在使用服务器模式
   (``delv +ns``)时，用于发现根名字服务器。如果未指定这个选项，将使用内置的
   根提示信息。

.. option:: +ns, +nons

   本选项切换名字服务器模式。在使用这个选项时， ``delv`` 进程实例化成为一个
   完全的递归解析器，并用其查找所请求的查询名和类型。打开这个选项同时也激活
   ``+mtrace`` ， ``+strace`` 和 ``+rtrace`` ，这样每个迭代查询都会被记录，
   包含来自每个权威服务器的全部响应消息。这些记录的消息将会象平常一样被写到
   ``stdout`` 而不是 ``stderr`` ，这样就能很容易地捕捉到全部跟踪信息。

   这是为了与 ``dig +trace`` 的行为相似，但是因为它使用与 ``named`` 同样的
   代码，它更精确地复制了一个递归名字服务器在没有缓存时处理一个递归查询的
   行为。

.. option:: +qmin[=MODE], +noqmin

   当与 ``+ns`` 一起使用时，本选项开启QNAME最小化模式。MODE的有效选项为
   ``relaxed`` 和 ``strict`` 。缺省时，QNAME最小化是关闭的。如果指定了
   ``+qmin`` ，但是省略了MODE，将会使用 ``relaxed`` 模式。
...
.. option:: +mtrace, +nomtrace
   ...
   本选项切换接收消息的日志。这产生 :program:`delv` 在执行解析和验证过程中收
   到的响应的详细导出结果。
...
.. option:: +strace, +nostrace
   ...
   本选项切换发出消息的日志。这产生 :program:`delv` 在执行解析和验证过程中
   发出的查询的详细导出结果。打开这个选项同时也激活 ``+mtrace`` 。

   这等效于在 "resolver" 日志类别中为 "packets" 模块设置调试级别为11。使用
   :option:`-d` 选项在系统范围设置调试级别为11产生同样的输出，但是也会影响
   其它日志类别。
