...
包括DNS的互联网协议族的规范文档是作为 `Request for Comments`_ （RFC）系
列的技术备忘的一部份被出版的。标准本身是由
`Internet Engineering Task Force`_ （IETF）和
`Internet Engineering Steering Group`_ （IESG）所定义。RFC可以在
https://www.rfc-editor.org/ 在线观看。

在阅读RFC时，请牢记 :rfc:`not all RFCs are standards <1796>` ，文件的
有效性也会随着时间的推移而改变。每个RFC都需要在其它文档的上下文中进行
解释。
