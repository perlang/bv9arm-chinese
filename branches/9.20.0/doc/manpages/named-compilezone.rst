...
与 :program:`named-checkzone` 不同，缺省时区内容不被严格检查。如果输
出是用作被 :iscman:`named` 装载的实际区文件，检查级别应当手动配置为
至少与 :iscman:`named` 配置文件中所指定的一样严格。

在输入上运行 :program:`named-checkzone` 优先于编译将确保使用
:iscman:`named` 缺省的要求来编译区。
...
.. option:: -C mode
   ...
   本选项控制装载时对区文件的检查模式。可能的模式有 
   ``check-svcb:fail`` 和 ``check-svcb:ignore`` 。

   ``check-svcb:fail`` 打开对 ``_dns`` SVCB记录的附加检查，而
   ``check-svcb:ignore`` 关闭这些检查。缺省是 ``check-svcb:ignore`` 。

.. option:: -i mode

   本选项对已装载区执行完整性检查。可能的模式为 ``full`` ，
   ``full-sibling`` ， ``local`` ， ``local-sibling`` 和 ``none`` （缺省）。
...
.. option:: -W mode
   ...
   （ :rfc:`4592` ）理解失败的结果。可能的模式为 ``warn`` 和
   ``ignore`` （缺省）。
