...
.. warning::
  ...
  请注意，这个版本的BIND 9当与在TLS上的区传送（XoT）相配合时，即当
  :any:`tls` 关键字与 :any:`primaries` 配合使用时，例如
  ``primaries { 192.0.2.1 tls tls-configuration-name; };`` ，不支持动
  态更新的转发，
...
:any:`auto-dnssec`
   参见 :namedconf:ref:`options` 中对 :any:`auto-dnssec` 的描述。
...
.. namedconf:statement:: update-policy
   ...
   个身份所更新。身份是由密钥决定的，这个密钥使用TSIG签名更
   ...
   必须被用于签名更新请求的TSIG密钥的名字。如果指定的名字是
